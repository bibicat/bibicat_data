...
public class D3zwwBookServiceImpl implements BookService{
	...
	public List<? extends BookBean> getBook_links(String path) {
		...
		try {
			while(null == doc) {
				doc = DocumentUtil.getDocumentProbablyProxy(path);
			}
		} catch (Exception e) {
			System.out.println("book:Connection refused: connect");
		}
		//解析dom
	...
	public List<? extends BookBean> getChapter_links(List<? extends BookBean> BookBeans) {
		for(BookBean bb: BookBeans) {
			...
			try {
				...
				Elements bookArticles = doc.select("div.book_list").select("ul").select("li").select("a");
				...
				if(data_total.intValue() == web_total.intValue()) {
					...
					for(;i < j;i++) {
						...
						while(null == article) {
							try {
								article = DocumentUtil.getDocumentProbablyProxy(article_link);
							} catch (Exception e) {
								e.printStackTrace();
							}
						}
